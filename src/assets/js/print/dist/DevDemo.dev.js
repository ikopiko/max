...
function sendData() {
  ...
  checkPrinterStatus(function (text) {
    if (text == "Ready to Print") {
      selected_printer.send(format_start + 'BLA' + format_end, printComplete, printerError);
    } else {
      printerError(text);
    }
  });
...
function checkPrinterStatus(finishedFunction) {
  selected_printer.sendThenRead("~HQES", function (text) {
    ...
    var pause = text.charAt(84); // check each flag that prevents printing
